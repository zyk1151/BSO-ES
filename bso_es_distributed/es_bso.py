...
from .dist_bso import MasterClient, WorkerClient
...
CurState = namedtuple('CurState', ['x', 'EpRewMean', 'EpRewStd', 'EpLenMean',
                                   'EvalEpRewMean', 'EvalEpRewStd', 'EvalEpLenMean',
                      'EvalEpCount', 'num_generation', 'EpisodesThisIter', 'EpisodesSoFar',
                                   'TimestepsThisIter', 'TimestepsSoFar', 'TimeElapsedThisIter', 'TimeElapsed', 'ob_stat_mean', 'ob_stat_std'])

BestState = namedtuple('BestState', ['x', 'y', 'mean', 'std'])
...
def run_master(relay_redis_cfg, log_dir, noise, node_id="node_0"):
    logger.info('nodeid:{} run_es_master: {}'.format(node_id, locals()))
    es_master = WorkerClient(relay_redis_cfg, node_id)
    exp = es_master.get_experiment("es master")

    # save exp
    with open(log_dir+"exp.json", "w") as fp:
        json.dump(exp, fp)
    ...
    timesteps_so_far = 0
    tstart = time.time()
    # es_master.declare_experiment(exp)
    ...
    while True:
        ...
        # get a new theta from BSO master
        if es_master.get_flag():
            theta = es_master.get_new_task()
            es_master.set_flag(False) # modify the new bso task flag
            modify_flag = 1
            pass
        else:
        ...
        curr_task_id = es_master.set_current_task(Task(
            params=theta,
        ...
        while num_episodes_popped < config.episodes_per_batch or num_timesteps_popped < config.timesteps_per_batch:
            # Wait for a result
            task_id, result = es_master.pop_result()
        ...
        state = CurState(
            x=theta,
            EpRewMean=returns_n2.mean(),
            EpRewStd=returns_n2.std(),
            EpLenMean=lengths_n2.mean(),
            ...
            num_generation=curr_task_id,
            EpisodesThisIter=lengths_n2.size,
            EpisodesSoFar=episodes_so_far,
            TimestepsThisIter=lengths_n2.sum(),
            TimestepsSoFar=timesteps_so_far,
            TimeElapsedThisIter=step_tend - step_tstart,
            TimeElapsed=step_tend - tstart,
            ob_stat_mean=ob_stat.mean,
            ob_stat_std=ob_stat.std
        )
        es_master.set_current_stat(state)
        es_master.set_current_flag(True)

        if state.EvalEpRewMean is not np.nan:
            if state.EvalEpRewMean > best_y:
                best_y = state.EvalEpRewMean
                best_state = BestState(x=theta, y=state.EvalEpRewMean, mean=ob_stat.mean, std=ob_stat.std)
                es_master.set_best_flag(True)
                es_master.set_best_stat(best_state)
                pass


        tlogger.record_tabular("EpRewMean", state.EpRewMean)
        tlogger.record_tabular("EpRewStd", state.EpRewStd)
        tlogger.record_tabular("EpLenMean", state.EpLenMean)

        tlogger.record_tabular("EvalEpRewMean", state.EvalEpRewMean)
        tlogger.record_tabular("EvalEpRewStd", state.EvalEpRewStd)
        tlogger.record_tabular("EvalEpLenMean", state.EvalEpLenMean)
        ...
        tlogger.record_tabular("EvalEpCount", state.EvalEpCount)

        tlogger.record_tabular("Norm", float(np.square(policy.get_trainable_flat()).sum()))
        tlogger.record_tabular("GradNorm", float(np.square(g).sum()))
        tlogger.record_tabular("UpdateRatio", float(update_ratio))

        tlogger.record_tabular("EpisodesThisIter", state.EpisodesThisIter)
        tlogger.record_tabular("EpisodesSoFar", state.EpisodesSoFar)
        tlogger.record_tabular("TimestepsThisIter", state.TimestepsThisIter)
        tlogger.record_tabular("TimestepsSoFar", state.TimestepsSoFar)
        ...
        tlogger.record_tabular("TimeElapsedThisIter", state.TimeElapsedThisIter)
        tlogger.record_tabular("TimeElapsed", state.TimeElapsed)
        tlogger.record_tabular("ModifyByBSO", modify_flag)
...
def run_worker(relay_redis_cfg, noise, node_id, *, min_task_runtime=.2):
    logger.info('run_es_worker: {}'.format(locals()))
    assert isinstance(noise, SharedNoiseTable)
    es_worker = WorkerClient(relay_redis_cfg, node_id)
    exp = es_worker.get_experiment()
    ...
    while True:
        task_id, task_data = es_worker.get_current_task()
        ...
        if rs.rand() < config.eval_prob:
            ...
            logger.info('Eval result: node_id={} task={} return={:.3f} length={}'.format(node_id, task_id, eval_return, eval_length))
            es_worker.push_result(task_id, Result(
                worker_id=worker_id,
                noise_inds_n=None,
                returns_n2=None,
                signreturns_n2=None,
