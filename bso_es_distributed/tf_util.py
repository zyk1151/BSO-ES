...
def dense(x, size, name, weight_init=None, bias=True):
    ...
    # w = tf.get_variable(name + "/w", [x.get_shape()[1], size], initializer=tf.random_uniform_initializer)
    ret = tf.matmul(x, w)
    if bias:
        b = tf.get_variable(name + "/b", [size], initializer=tf.zeros_initializer)
        # b = tf.get_variable(name + "/b", [size], initializer=tf.random_uniform_initializer)
        return ret + b
    else:
        return ret
